    ...
    def join_samplers(self, num_cores):
        ...
        for identifier in data_list:
            ...
            if self.dimension == "Histogram":
                ...
                combined_data[identifier]["mean_charge"] = np.sum(data_list[identifier]["mean_charge"]) / num_atoms if num_atoms > 0 else np.nan
                ...
                combined_data[identifier]["std_charge"] = 0 # TODO: fix std calculation
...
def plot_hist(link_data: str, axis: Axes | bool=True, identifiers = [], colors = [], std=False, mean=False, density=False, plot_kwargs = {}):
    ...
    for i, identifier in enumerate(identifiers):
        ...
        if density:
            hist = hist / data[identifier]["num_atoms"]
        std_hist = data[identifier]["std_hist"] if std else None
        mean_value = data[identifier]["mean_charge"] if mean else None
