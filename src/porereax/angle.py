...
The module provides :class:`AngleSampler` to sample bond angle histograms for
specified atom structures. It supports filtering by specific A-B-C triplets or
sampling all angles formed by the central atom B.
"""
...
class AngleSampler(AtomSampler):
    ...
    angle : str
        Angle of interested atoms. Supported: "all", "A-B-C" where A, B, C are
        atom identifiers.
    """
    ...
    def __init__(
        ...
        if angle != "all":
            if len(angle.split("-")) != 3:
                raise ValueError(
                    "AngleSampler 'angle' parameter must be 'all' or in the "
                    "format 'A-B-C'."
                )
            ...
            for atom in angle_atoms:
                if atom not in atom_lib:
                    raise ValueError(
                        "AngleSampler 'angle' parameter contains unknown atom "
                        f"identifier '{atom}'."
                    )
        ...
        self._input.update(
            {"num_bins": num_bins, "range": self._range, "angle": angle_list}
        )

        # Remove atom structures with less than 3 atoms or not matching A-B-C
        # if specified
        molecules_to_remove = []
    ...
    def _combine_identifier(self, identifier: str, data: dict) -> dict:
        ...
        worker_counts = np.array(data["num_angles"])
        worker_sums = np.array(data["mean_angle"])
        ...
        worker_frames = np.array(data["num_frames"])[valid_workers]
        worker_hists = (
            np.array(data["hist"])[valid_workers] / worker_frames[:, np.newaxis]
        )
        hist_std = (
