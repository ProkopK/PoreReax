...
The module provides :class:`BondLengthSampler` to sample bond length
histograms (in Angstroms) or ReaxFF bond order histograms for specified bonds.
"""

import numpy as np

import porereax.utils as utils
from porereax.meta_sampler import _BOND_SAMPLER_INIT_PARAMS, BondSampler
...
class BondLengthSampler(BondSampler):
    ...
    def __init__(
        ...
        if not isinstance(dimension, str) or dimension not in valid_dimensions:
            raise ValueError(
                f"BondLengthSampler does not support dimension {dimension}"
            )
        ...
        if (
            ...
            raise ValueError(
                "BondLengthSampler requires a 'range' parameter as a list or "
    ...
    def sample(
        ...
        for identifier in self._bonds:
            b_mask = (
                bond_mask[identifier]
                & position_mask[bond_topology[:, 0]]
                & position_mask[bond_topology[:, 1]]
            )
            ...
            if self._dimension == "Bond Length":
                bond_lengths = np.linalg.norm(
                    utils.min_image_convention(
                        position[:, 0, :] - position[:, 1, :], self._box
                    ),
                    axis=1,
                )
                hist, _ = np.histogram(
                ...
                bond_orders = (
                    ...
                    if "Bond Order" in frame.particles.bonds
                    else np.zeros(frame.particles.bonds.count)
                )
