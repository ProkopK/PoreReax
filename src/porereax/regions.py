...
def get_region_function(
    region: str, box: NDArray[np.float64], system_properties: dict | None = None
    ...
    if region == "Box":
        ...
        if system_properties is None:
            raise ValueError(
                "System properties must be provided for the 'Reservoir' region."
            )
        return lambda coords: reservoir_region(coords, system_properties)
    elif region == "Pore":
        if system_properties is None:
            raise ValueError(
                "System properties must be provided for the 'Pore' region."
            )
        ...
        if region == "Wall":
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid layer thickness specified in region identifier: {region}"
                ) from e
        else:
            raise ValueError(f"Unknown region identifier: {region}")
        if system_properties is None:
            raise ValueError(
                "System properties must be provided for the 'Wall' region."
            )
...
def reservoir_region(
    coords: NDArray[np.float64], system_properties: dict
) -> NDArray[np.bool_]:
    """
    ...
    return ((coords[:, 2] <= lower_bound) | (coords[:, 2] >= upper_bound)) & (
        distance_from_center >= pore_radius
    )
...
def wall_region(
    coords: NDArray[np.float64], system_properties: dict, layer_thickness: float
) -> NDArray[np.bool_]:
    """
    ...
    reservoir_wall_left = (
        (coords[:, 2] >= lower_bound_left)
        & (coords[:, 2] <= upper_bound_left)
        & (distance_from_center >= pore_radius - layer_thickness)
    )
    reservoir_wall_right = (
        (coords[:, 2] >= lower_bound_right)
        & (coords[:, 2] <= upper_bound_right)
        & (distance_from_center >= pore_radius - layer_thickness)
    )
    pore_wall = (
        (coords[:, 2] >= lower_bound_left)
